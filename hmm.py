...
def viterbi(observations, emission_matrix, initial_pro, transition_matrix):
    ...
    cheat[0]=initial_pro[0]*(emission_matrix[0][0] if observations[0] == 'lose' else emission_matrix[0][1])
    fair[0]=initial_pro[1]*(emission_matrix[1][0] if observations[0] == 'lose' else emission_matrix[1][1])
    for i in range(1,len(observations)):
        a = cheat[i - 1] * transition_matrix[0][0] * (emission_matrix[0][0] if observations[i] == 'lose' else emission_matrix[0][1])
        b = fair[i - 1] * transition_matrix[1][0] * (emission_matrix[0][0] if observations[i] == 'lose' else emission_matrix[0][1])
        cheat[i]=max(a,b)
        
        print(cheat[i])
        a = cheat[i - 1] * transition_matrix[0][1] * (emission_matrix[1][0] if observations[i] == 'lose' else emission_matrix[1][1])
        b = fair[i - 1] * transition_matrix[1][1] * (emission_matrix[1][0] if observations[i] == 'lose' else emission_matrix[1][1])
        fair[i]=max(a,b)
        print(fair[i])
